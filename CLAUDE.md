...
- **Core pipeline:** `backend/core/propagator.py`, `tle_fetcher.py` (fetch + `slice_to_shell` + `build_starlink_shell`), `coordinate_transforms.py`, `conjunctions.py` (ConjunctionScreener + run_screen + fine_filter; 7.2 SFS-ellipsoid path + co-located suppression + de-dupe), `screening_volumes.py` (7.2 — SFS Table 3 RTN ellipsoids + `regime_for`), `demo_seed.py` (synthetic crosser + dense shell generator)
...
- **Tests:** `tests/` (409 tests across 9 test files)
...
## Current Status
- **Phase:** Final Sprint underway — Phase 6 COMPLETE (6.0–6.10); **Phase 7 underway: 7.0 (live data) + 7.1 (scale profile) + 7.2 (SFS ellipsoid volumes + co-located suppression) done**, next 7.3 (performance pass). Also: interactive core of 9.1 (conjunction UX) pulled forward.
...
- **Next steps:** Phase 7.3 (performance pass). **7.1 profile reordered Phase 7** (`task_logs/task_7_1_scale_profile.md`): the **fine stage is 82–87% of wall time** → 7.3 priorities are fine-stage C++ batching (primary), then the coarse→medium boundary (memory fix, #3), then `run_in_threadpool` (#4); plus the radial coarse-pad tightening 7.2 deferred. Full catalog = 258 s / 4.5 GB → batch-only until 7.3; **demo operating point `MAX_SATS=300` = 2.6 s @ 24 h**. **7.2 done** (`task_logs/task_7_2_screening_volumes.md`): screening now uses the **SFS RTN ellipsoid volumes** (radial-tight 0.4 km / along-cross-track loose) + co-located suppression + de-dupe — real Starlink (300) → 7 conjunctions; live stations → 0 events / 46 docked suppressed (was 57). ⚠ **The SFS default excludes the synthetic crosser** (radial-dominated), so a populated demo needs a **real Starlink shell**, not `stations` + seed.
- **Tests:** 409 passing + 1 skipped (opt-in live fetch) across 9 test files — suite runs offline/deterministic. Frontend JS has no automated tests
