...
- **Next steps:** Phase 6.5 (RTN coordinate transform — pure Python, ~20 lines) then 6.6 (fine filter). Full breakdown in `progress/week6_plan.md`. (6.0–6.4 done; C++ screening core complete: propagate_batch + coarse_filter + medium_filter.)
- **Tests:** 321 passing + 1 skipped (opt-in live fetch) across 7 test files — suite runs offline/deterministic. Frontend JS has no automated tests
