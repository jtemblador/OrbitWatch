...
- **Next steps:** Phase 6.3 (C++ medium filter — time-stepped pair distance scan; design decision pending: run the coarse cut internally to avoid the C++→Python pair round-trip). Full breakdown in `progress/week6_plan.md`. (6.0–6.2 done.)
- **Tests:** 309 passing + 1 skipped (opt-in live fetch) across 7 test files — suite runs offline/deterministic. Frontend JS has no automated tests
