 ...
 *
 * This file is the Python boundary only: validation, GIL handling, and
 * Python<->C++ conversion. The screening engine itself (the coarse cut and
 * the 6.3 time-major medium scan) lives in src/screening.cpp (Phase 10.2).
 */
...
// (The pure-C++ screening engine — the coarse cut and the 6.3 medium scan —
// lives in screening.cpp since the Phase-10.2 split; this boundary helper
// stays here because it touches Python objects.)
