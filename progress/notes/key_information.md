...
**10.2 time sieve — build/validation findings (Jul 9).** The sieve is IN the engine (`screen_pairs(time_filter=True)` / `run_screen(sieve=True)`, default off until 10.6): C++ anchors ≡ the Python oracle to 2e-11 (locked by `test_cpp_anchors_match_python_oracle` via `_sieve_anchors`); events byte-identical on/off on every catalog (incl. **253,392==253,392** on full Starlink); medium scan 4.4–6.2× faster, cascade ~2× (fine now dominates → Stage 2). Durable gotchas: **(1) on a single co-altitude shell the sieved ROW set is byte-identical** — every medium flag sits near a node crossing (flag-run width and node-window width both scale with 1/sin I_R); spurious windows are a *cross-shell* phenomenon, so a "sieve bites" test needs a two-shell fixture. **(2) `run_screen`'s event sort must be a TOTAL order** (miss, sat1, sat2, tca) — byte-equal miss ties are real on symmetric geometry and a miss-only sort leaks the scan's internal row order (sieved vs full paths disagreed on tie order while set-identical). **(3) Every geometric quantity the C++ interpolates that the oracle computes exactly needs its own measured bound** — the node-path nonlinearity (1/sinIR-amplified) was the one unbounded term; cured with the midpoint chord-deviation margin (the same measured-curvature discipline as the anchors). **(4) v1 keeps unconditional used-sat propagation** — skipping propagation for inactive sats is the silent-failure bug class (NaN → quietly closed window); the per-sat refcount is a flagged, measured follow-up (~15 s of the remaining ~20 s medium at the CI point).

**10.6 flip live + full-catalog cap + frontend perf (Jul 15) — PHASE 10 DONE, durable bits.** Production runs `fused+sieve+refine` (`build_snapshot._ACCEL`); the full ~16k catalog is screened + displayed (cap gone). Gated byte-identical **in CI on live data** (Tier-1 308==308, Tier-2 3,887==3,887). Durable lessons: **(1) A CI gate should re-run its (slow) reference only when the thing it guards CHANGES.** The gate re-runs the OLD classic cascade (~40 min on the 4-vCPU runner — Tier-2's sieve-off medium is the long pole) to prove the fast engine matches. That guards CODE, not data — so scheduled cron rebuilds SKIP the gate (`if: … && github.event_name != 'schedule'`) and just re-screen with the validated engine (~10 min); the full gate runs on push/dispatch. **(2) A full-scale A/B must keep BOTH sides memory-safe.** Comparing sieve-off vs sieve-on at 16k means the sieve-off side ALSO needs the C++ refine (`ab_screen` fused baseline honors `--refine`), or it re-materializes the ~13 GB Python fine dicts (#7) the phase removed — Tier-2 ran at 3.24 GB, not OOM. **(3) "Screen everything" ≠ "render everything."** Browser lag scaled with displayed conjunction *participants* (~6,900/frame at full catalog vs ~557 at 5k) — it's the client-side SGP4, not Cesium's draw. Cap the DISPLAY (closest-N conjunctions, `CONJ_DISPLAY_CAP`) not the screen; header shows "closest N of total" to stay honest; the full set stays in the snapshot + archive. **(4) Focus isolation is a PROPAGATION lever** — masking the web worker to the focused handful (`isolationMask`) is where the CPU actually drops (2–3 sats/frame vs ~800), not just hiding dots. Frontend UX: a 3-level zoom stack (All → Conjunction View → TCA View) with Escape walking UP and a POV badge; unified point styling (hover > selected/pair > plain) so the three emphasis sources never fight.

**10.5 flip property tests (Jul 9) — durable bits.** `TestFlipContract` locks the exact 10.6 path (`classic == fused+sieve+refine`) offline on a hostile composite fixture. Durable lessons for anyone touching these: **(1) a "hostile" fixture is worthless until you VERIFY its adversarial cases participate** — a high-drag/eccentric singleton that silently decays or coarse-filters out tests nothing; measure that they propagate the full window AND appear in medium windows before trusting the framing (both here do: 33/70 windows). **(2) Assert BOTH stages engaged, not one** — `n_survivors < n_windows` proves refine; proving the sieve needs `flip n_windows < classic full-scan n_windows` (else a regression that silently disables the sieve still passes identity). **(3) The sieve's event-level zero-margin bite is UNCONSTRUCTIBLE on synthetic geometry** — every sieve conservatism except the 0.5° base is a constructive bound, so zeroing margins shrinks rows but drops 0 events on well-conditioned shells (10.1b measured event drops 1/59/256/351 only on real-catalog pathologies); assert the row-level bite, document why, leave the event-level check to real data (10.6's in-CI A/B) — don't fake it. **(4) Verify a test bites by COMPILING a mutation** (remove the sieve ±1-step pad → 1 lock red; swap the refine T/N ellipsoid axes → 3 red), then `git checkout` the engine + rebuild — a green test that's never been made to fail proves nothing. **(5) A branch that cannot change events isn't worth a fixture** — the C++ pre-cut's decayed-row-survives path is untested because even a wrong decision there yields identical events (Python drops decayed rows too).

**10.4 C++ fine stage (Jul 9) — durable bits.** Stage 2 is IN the engine (`screen_pairs(refine=True)` / `run_screen(refine=True)`, default off until 10.6): OpenMP Newton refinement + a **superset pre-cut** — C++ decides only *which rows might pass the report cut* (per-pair ε-inflated RTN ellipsoid / Euclidean threshold+ε); `fine_filter_batch` re-refines the survivors so every reported float is the validated Python path's → events byte-identical **by construction**. Full 15,709-sat/24 h screen: ~25 GB-impossible → **304.6 s / 5.38 GB**. Durable gotchas: **(1) a conservative pre-cut must match the report cut's SHAPE** — a gross-ball bound was safe but useless (21.4% survivors; SFS radial axis is 0.4 km vs the 51 km gross); the ellipsoid with the same ε is ~200× tighter (0.12–0.36%). **(2) `sgp4()` mutates the elsetrec — concurrent calls on a shared satrec are NOT thread-safe**; any parallel propagation needs private copies per task (cheap: ~2 KB POD vs ~12 evals/row), and determinism needs results indexed by input position + serial compaction (thread-count invariance is test-locked). **(3) A per-row C++ transcription of vectorized NumPy (same update, same `where` guards, same clip) can be bit-identical in practice** — measured |ΔTCA| exactly 0 on 934k rows — but keep the ε margin anyway; the superset contract is what makes FP drift a non-event. **(4) Python's `max(a, b, key=…)` is FIRST-wins on ties** — replicating pair-volume selection in C++ means `use_j = rc_j > rc_i` (strict), not `>=`. **(5) Euclidean-mode survivors ≈ the kept events themselves** (the cut keeps ~25% of Starlink windows) — the big survivor collapse is an SFS-ellipsoid property, so memory claims must be quoted per report-cut shape. `_refine_oracle` is the permanent oracle hook (twin of `_sieve_anchors`).

**10.3 formal Stage-1 validation (Jul 9) — durable bits.** Events byte-identical on catalogs the sieve never saw (fresh elements median 0.54 d old; Starlink +230 new sats incl. a week-old orbit-raising batch at ~305 km): 312 / 312,042 / 4,445 at CI-point/full-Starlink/full-active. Same-day rows: Stage-1 **2.11×/2.22× total, RSS −23%/−26%** (5k/10k); post-sieve **fine = 73–81% of the cascade** → 10.4. **(1) CelesTrak TLS-blocked (again — 8.3 was the first): the working fallback is ONE Space-Track bulk `gp` query** (`/class/gp/decay_date/null-val/epoch/>now-30`, ~36 MB / ~32k objects) → `_coerce_numeric` → the canonical `GPFetcher._parse_json` → slice (active = the cached population's NORAD ids, preserving the group definition; starlink = name prefix) → `_cache_to_parquet`. Promote into `tle_fetcher` only if it recurs. **(2) Don't conflate the two medium metrics:** wall speedup is 4.9–6.6× while pair-step *work* fell ~41× — the sieved scan still propagates every used sat every step (v1's deliberate floor); quoting −41× for wall time would be wrong. **(3) `ru_maxrss` is process-wide high-water** — in any two-run A/B process the second run's RSS reading is contaminated by the first; per-path memory numbers must come from separate processes (profiler invocations). **(4) `scripts/ab_screen.py` is the standing identity harness** (exit 0 iff events byte-identical; `--baseline classic` at ≤5k, `--baseline fused` at full scale to avoid re-materializing the classic pair list) — 10.6's CI A/B calls it.
