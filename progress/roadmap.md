...
> tuples** on the full active catalog (the full screen **cannot fit the 16 GB CI runner at all**:
> ~25 GB extrapolated; the 9.4 "38 min" was memory pressure, not CPU). ⚠ **Measured in 10.1a:** that
> pair list is only ~5.4 GB of the ~25 GB — the **bigger** memory term is the fine stage's per-window
> result dicts (`scaling_tracker #7`), so fitting the full catalog needs BOTH the fused stage (1a, done)
> AND the C++/streamed fine stage (10.4); pair-list removal alone is necessary, not sufficient.
...
- [x] **10.1a Fused C++ coarse+medium stage (memory, low-risk half of Stage 1)** — `orbitcore.screen_pairs`: one GIL-released call does the coarse altitude-band cut AND the time-stepped medium scan, so the survivor-pair list lives only in C++ and never materializes as Python tuples (closes the *Python-materialization* part of `scaling_tracker #3`/`#8`). The validated 6.3 scan was extracted into a shared `run_medium_scan` helper (used by both `medium_filter` and `screen_pairs`) — **`medium_filter`'s 143 tests still pass byte-identical**, proving the refactor is behavior-preserving. `run_screen(fused=True)` wires it, gated + `timings`-hooked; **byte-identical events** proven fused vs non-fused (Euclidean shell + a real 2,338-sat active slice, 78 SFS events identical). **Measured — corrects the plan's memory claim:** the Python pair list is only ~0.35 GB at the CI slice (~5.4 GB at full 16k), i.e. **NOT** the bulk of peak RSS — the fine-stage per-window dicts (`#7`) are the bigger chunk. CI-point RSS 1.71 → **1.34 GB**; full 16k needs the fine-stage memory (10.4 / `#7`) too, not 1a alone. 1a is the clean pair-list removal **and** the fused primitive 10.1b builds the time filter into. +6 tests; 562 passing. See `task_logs/task_10_1a_fused_stage.md`.
- [x] **10.1b Time-filter gate: spec + no-skip validation BEFORE any C++ (GO)** — the H-C-R Filter III construction validated exhaustively: `fine_filter_batch` refines **all** flagged windows and every real event is checked at its **TCA** against the predicted node windows — **1,403,999 real events across 4 catalogs, 0 uncovered at a 0.25° base margin** (ship 0.5° = 2× headroom); the zero-margin mutation bites on every catalog (1/59/256/351); the **5 km Euclidean (SOCRATES-validation) mode holds too** (1,681 sub-5 km events, 0 uncovered). **Realized coverage: 2.47% of pair-steps at the CI slice (~41×), 0.77% full Starlink (~130×), 0.60% full active (~168×)** — pinned to the near-coplanar floor, so margins are ~free. The naive filter fails **four separately-measured ways**, each cured in the validated construction: (1) the contract is TCA-coverage, never flag-time (medium flags are sampled steps ~1 step from the crossing → C++ pads windows ±1 step *in time*); (2) anchor at **screen start**, never epoch (drag-secular divergence: 35° at a 21.8-day epoch age — and the satrec's own rates do NOT fix it, tested); (3) **equinoctial chord rates** (λ=ω+M; the osculating (ω,M) split is ill-conditioned below e≈0.002 — an ordinary e=0.0011 sat broke the M-unwrap); (4) **per-sat measured curvature margins** (midpoint second difference — reentering 143-km-perigee sats need ~6° where normal sats need ~0.1°; flat margins either miss them or erase the win). Deliverables: `week10_planning/stage1b_timefilter_spec.md` (the spec) + `time_filter_gate.py` (**executable spec = the 10.2/10.5 oracle**); `Satrec` gained read-only `mdot/argpdot/nodedot`. Honest framing: Stage 1 total ≈ 1.5–2× at the CI point (fine untouched); the 41–168× is what makes the *full-catalog* medium scan trivial. 563 passing. See `task_logs/task_10_1b_timefilter_gate.md`.
- [x] **10.2 The C++ time sieve — identical events, medium scan 4.4–6.2× faster** — `screen_pairs(time_filter=True)` → `run_screen(sieve=True)` (default **off** until 10.6 flips production): per-pair time intervals precomputed from the validated construction (3 propagations/sat → rv2coe → **equinoctial chord rates + measured curvature margins**, C++ anchors **≡ the Python oracle to 2×10⁻¹¹** and locked by test via `_sieve_anchors`; crossings from a linear phase model anchored on exact osculating u₀, EoC-bound widened, ±1-step pad; per-sat windows intersected → CSR step ranges + activation index), consumed by an **activation-list time-major scan** (one shared `eval_pair` lambda with the classic path; v1 deliberately still propagates all used sats — skipping propagation is the silent-failure bug class; refcount = flagged follow-up). **Events byte-identical on every catalog: CI slice 366==366 (medium 107→20.7 s, total 168→~80 s, RSS 1.31 GB), full Starlink 253,392==253,392 (110.9→25.3 s), full active 3,816==3,816 (382.6→62.0 s).** Total ~2× as 10.0 predicted — the fine stage now dominates (→10.4). Incidental fix: `run_screen`'s event sort is now a **total order** (byte-equal miss ties on symmetric geometry leaked scan order). **Review round:** the one unbounded error source — node-path nonlinearity in the interval builder (the oracle recomputes geometry exactly; the C++ interpolates) — now bounded by the measured midpoint chord deviation (+~2 s, events still identical). Prep commits: the engine split (`screening.cpp/h`, pure C++, Stage-2/OpenMP-ready) + one-topic-per-file bindings (`bind_satrec/bind_propagation/bind_screening` + 37-line entry), each verified byte-identical before any sieve code. +5 tests (`TestTimeSieve` incl. the anchors-vs-oracle lock + a two-shell spurious-window-drop proof); **568 passing**. See `task_logs/task_10_2_time_sieve.md`.
- [x] **10.3 Validate + re-profile Stage 1 — formal pass on catalogs the sieve never saw (Jul 9)** — caches refreshed to **median epoch age 0.54 d** via a Space-Track bulk `gp` fallback (CelesTrak TLS-blocked; same 15,928-id active population, Starlink +230 new sats incl. a Jul-2 batch still orbit-raising at ~305 km — the drag-heavy class that stressed 10.1b's margins). **Events byte-identical at every scale: CI point 312==312 (classic vs Stage-1), full Starlink 312,042==312,042, full active 4,445==4,445**; full suite (incl. the C++-anchors-vs-oracle lock) green on the fresh caches; 10.2's node-aliasing residual risk — 0 differences on all-new geometry. **Same-day profile rows:** classic re-baselines reproduce 10.0 (CI 146.8 s/1.72 GB, 10k 874.9 s/8.69 GB) → Stage-1 **2.11×/2.22× total, RSS −23%/−26%**, medium wall 4.9–6.6× (≠ the −41× pair-step metric — the v1 propagation floor dominates the sieved scan); **post-sieve fine = 73–81% of the cascade** (10.4's case, measured three ways); full-active 24 h still needs 10.4's streamed fine stage to fit. Shipped **`scripts/ab_screen.py`** — the load-once/screen-twice/diff-events harness (exit 0 iff byte-identical) that 10.6's CI A/B runs. See `task_logs/task_10_3_stage1_validation.md`.
- [x] **10.4 C++ fine stage (Stage 2) — identical events, the first-ever full-catalog 24 h screen (Jul 9)** — `screen_pairs(refine=True)` → `run_screen(refine=True)` (default **off** until 10.6): the 7.3 Newton refinement transcribed per-row into the engine (**OpenMP across rows, private satrec copies** — `sgp4()` mutates; thread-count invariance test-locked) with a **SUPERSET pre-cut, not a numbers port**: C++ only decides which rows might pass the report cut (per-pair **margin-inflated RTN ellipsoid** — a gross-ball cut measured out at 21.4% survivors vs the ellipsoid's 0.12–0.36%; Euclidean threshold+ε otherwise; propagation failures handed back to Python), and `fine_filter_batch` re-refines just those survivors so every reported float comes from the validated Python path — **events byte-identical by construction**. ε is measured, not guessed: |ΔTCA| **exactly 0**, |Δmiss| ≤ 5.7e-14 km on 934k real rows (0.01 km default ≈ 12 orders of headroom). **Identity on every catalog** (312 / 312,042 / 4,445 == , incl. classic-vs-full-flip) + **thread invariance**. **Rows:** CI point 146.8→**32.1 s (4.6×)** RSS 1.72→**0.50 GB**; 10k 874.9→**118.1 s (7.4×)** RSS 8.69→**1.89 GB**; **full 15,709 / 24 h: ~25 GB-impossible (10.0) → 304.6 s / 5.38 GB** (17.07 M windows refined in C++, 9,052 events) — closes `scaling_tracker #7` by construction; medium is now ~88% of the remaining full-catalog cost (refcount = the known next lever). `_refine_oracle` = the permanent oracle hook; profiler/ab_screen gained `--refine`. +8 tests (`TestCppFine`); **576 passing 3× clean**. See `task_logs/task_10_4_cpp_fine_stage.md`.
- [x] **10.5 Mutation-checked property tests for the flip (Jul 9)** — `TestFlipContract` (5) + a **hostile composite fixture** (three shells at different altitudes/inclinations for real cross-shell geometry + a high-drag and an e=0.03 spanning-shells singleton — both *verified* to propagate the full window and appear in medium windows, 33/70, not silently decay). Locks the EXACT 10.6 contract offline: `classic == fused+sieve+refine` event-for-event (1,826 events) with **both stages asserted engaged** (sieve dropped 1,088 windows; refine dropped rows), the same through the SFS ellipsoid path (149 events), flip-path run-to-run determinism (OpenMP/scan-order), sieve-margin wiring, and a committed **refine pre-cut mutation bite** (deflate the ellipsoid to 40% → rows the report cut keeps go missing). **Every lock proven to bite** by compiled mutations (removed the sieve ±1-step pad → 1 lock red; swapped the refine T/N axes → 3 red; engine reverted). Honest limit: the sieve *event-level* zero-margin bite is unconstructible on synthetic geometry (constructive bounds; 10.1b measured it only on real-catalog pathologies) — row-level bite asserted, event-level left to 10.6's in-CI A/B, no fake assertion committed. Two review rounds (fixture honesty → assertion sensitivity). **581 passing 3× clean.** See `task_logs/task_10_5_property_tests.md`.
- [x] **10.6 CI A/B gate + production flip + full-catalog cap lift + frontend perf/UX round (Jul 15) — PHASE 10 COMPLETE, LIVE** — `build_snapshot.py` flipped to `fused+sieve+refine`; `deploy.yml` fetches the active catalog once → **two-tier A/B gate** (`set -e` fails the build → no deploy: Tier 1 `classic == fused+sieve+refine` at 5k; Tier 2 `fused+refine vs fused+sieve+refine` on the full catalog at 6 h, isolating the sieve, **memory-safe** because `ab_screen.py`'s fused baseline now honors `--refine` — ran at 3.24 GB, not the ~13 GB OOM) → screen the **FULL ~16k catalog** (cap removed). **Gated byte-identical IN CI on live data: Tier-1 308==308, Tier-2 3,887==3,887**; first full-catalog CI screen green (16,072 sats / 7,769 conj / 1.29 MB gz); live site verified. Pre-flight measured full 24 h = 307 s / 5.38 GB / 1.29 MB gz (under budget) → lift to full. **Cron gate-skip:** scheduled rebuilds skip the ~40-min gate (it guards code, not data) → 3×/day cron back to ~10 min; the full gate still runs on push/dispatch. **Frontend round** (full catalog lagged weak machines — ~6,900 propagated participants/frame): display cap to the closest 500 conjunctions (screen stays complete; header "closest N of total"), **focus isolation** (worker masked to the involved 2–3 sats), occlusion throttle + 15 fps; plus the UX — Escape zoom stack (All → Conjunction View → TCA View) with a POV badge, pair highlight, hover emphasis, rewind/pause transport + spacebar, TCA-time ground marker, Labels toggle removed (auto-labels the isolated handful), duplicate-trail fix. `scaling_tracker #3/#7/#8` resolved. See `task_logs/task_10_6_ci_flip_cap_and_ux.md`.

- [x] **10.7 Post-launch code review + hardening (Jul 15)** — a full-project review (four parallel reviewers over the C++ core, backend, frontend, CI) → 5 real findings fixed: (1) **`elsetrec::satnum` buffer overflow** — `char[6]` but `sgp4init` writes up to 9 digits in; overflows once catalog ids pass 99999 (≈ now). Widened `[6]→[10]`, copies bounded, +2 mutation-proven regression tests. Re-vendoring `SGP4.cpp/.h` must re-apply. (2) **Engine code reached prod ungated** — `deploy.yml`'s push filter is `frontend/**` only and a push there REUSES the snapshot, so `backend/`/`orbitcore/` changes skipped the gate until the next cron (which also skips it). New **`.github/workflows/ci.yml`** gates code at push/PR time (build + pytest + two-tier A/B on live data), read-only, no deploy. (3) **`/api/refresh` race** — reloaded the propagator cache without `_propagator_lock` + blocked the event loop; now off-loop + locked. (4) **A/B gate could pass on a truncated catalog** — added a `--min-objects 8000` floor (raw count, before `--max-sats`) to `ab_screen.py` + `build_snapshot.py`. (5) **Info-panel stale data** — showed the prior sat's numbers under a new name when propagation failed; now an explicit "no data" state (headless-verified). **583 passing.** See `task_logs/task_10_7_review_hardening.md`.

- [x] **10.8 Full-codebase review + fixes + UX round (Jul 21)** — 8 parallel agents over every subsystem; **engine + science verified sound** (C++ SGP4 bit-identical to the Vallado `sgp4` ref across 2,400 comparisons incl. deep-space; frames/GMST/RTN/conjunction-geometry cross-checked; SOCRATES genuinely same-method). 10 findings fixed (search stale-isolation; A/B gate screened-subset floor; startup catalog warm-up; co-located-suppression unrelated-near-miss guard; API `is_screenable` gate; `/positions`+`/track` off the event loop; `focusConjunction` draw-before-commit; cron gate-on-recent-code-change; pinned numeric stack; curl HTTP-status contract). A review-of-the-fixes caught a self-introduced refocus crash → fixed (isolate-before-reveal). UX: mode/selection-aware POV badge + full-orbit camera framing on selection (HEO now shows the whole ellipse). 583 tests; frontend headless-verified. See `task_logs/task_10_8_full_review_fixes_ux.md`.

**✅ Done — Phase 10 complete.** The full active catalog (~16 k) screens in the CI job within budget (~10 min cron / ~50 min gated, 5.4 GB), **events byte-identical** to the unfiltered cascade (proven in CI), and the live site shows the lifted cap with a smooth conjunction-first UI.
...
| ✅ Jul 15 | **Phase 10 COMPLETE (re-scoped by the 10.0 gate):** fused C++ time-sieve + OpenMP C++ fine stage — **full ~16k active catalog screened in CI and LIVE**, events byte-identical (gated in CI: 308==308 / 3,887==3,887), no-skip proven at event level; + conjunction-first UI perf/UX round | **Live, full catalog** |
