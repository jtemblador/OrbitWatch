...
- [ ] **10.2 Build the time filter into `screen_pairs` (C++)** — per-pair **time intervals** precomputed from the validated anchor construction (3 propagations/sat → rv2coe → chord rates + curvature margins → crossing times ± `δu/u̇_min` ± 1 step), intersected per pair; the medium scan then steps only inside them. ⚠ **Per-step membership checks are NOT acceptable** — they cost as much as the distance evaluation they replace (the win requires precomputed intervals). Flag-gated + `timings` hook; **byte-identical events** vs the unfiltered fused path is the acceptance gate, with `time_filter_gate.py` as the independent oracle.
