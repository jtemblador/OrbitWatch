...
- [ ] **8.1 SOCRATES fetcher** — pull CelesTrak conjunctions via the open `table-socrates.php` query endpoint (`CATNR`/`NAME`/`ORDER`/`MAX`; returns an HTML table) or the raw-CSV full-run download (no auth). CSV columns: `NORAD_CAT_ID_1/2`, `OBJECT_NAME_1/2`, `DSE_1/2`, `TCA`, `TCA_RANGE` (miss km), `TCA_RELATIVE_SPEED`, `MAX_PROB`, `DILUTION`. We use the first 8 (objects + **`DSE` — the epoch-match key** + TCA + range + speed); ignore `MAX_PROB`/`DILUTION` (Pc-related, de-scoped — note `MAX_PROB` uses *fixed assumed* covariance 100/300/100 m, not real covariance, so even SOCRATES isn't doing operational Pc). Cache to Parquet with a ~6–12 h TTL (SOCRATES updates 3×/day) — same fetch/serve pattern as `tle_fetcher.py`. **Parsing notes (columns + meaning verified Jun 24 against CelesTrak's official `socrates-format.php`):** the CSV is **RFC-4180 with a header row** (parse with the stdlib); `OBJECT_NAME` carries a **bracketed operational-status suffix** (e.g. `[+]` active / `[-]` inactive / `[P]` partial) — strip it for the clean name (it doubles as an active/inactive signal); `DSE` = *days from the GP-data epoch to the TCA* (exactly our epoch-match key); `MAX_PROB` uses a fixed 100/300/100 m covariance ellipse (confirmed — why it's not real Pc). **Verify at build:** exact bulk-CSV URL vs. per-object `table-socrates.php?CATNR=…&FORMAT=…` (a page-source check, not architectural).
- [ ] **8.2 Comparison logic** *(build as a **reusable function**, not a throwaway notebook — Phase 9's "matched SOCRATES?" badge (9.2) and the 9.9 evolution view reuse it)* — **fetch CelesTrak GP for every flagged object by `CATNR` (incl. debris / R-B secondaries — see scope note)**, screen in **legacy Euclidean mode at 5 km / 7 days** (NOT the SFS-suppressed default, which would drop events SOCRATES lists; SFS stays a separate lens), and check: do we detect the same events? Compare our TCA vs theirs, our miss vs theirs, matched on unordered pair `{id1,id2}` + TCA proximity (start tol ≈ TCA within a few min, miss within ~1 km for fresh epochs — tune in the plan). **Align the window:** screen the same 7-day forward span from the SOCRATES run we pulled (capture its run timestamp). **Confirm propagator parity:** WGS72 + AFSPC opsmode `'a'` (STK's SGP4 can differ sub-km — note any systematic bias in the report). **Epoch-match via `DSE` (the fix):** fetch current CelesTrak GP promptly, compute our element's age at the SOCRATES TCA, and confirm it equals their `DSE` — check **both** `DSE_1` and `DSE_2` (one per object) → *verified* same-snapshot comparison, no auth. **Step size:** MathWorks' reference uses 20 s for LEO; our velocity-aware no-skip bound tolerates 30–60 s, but keep the grid tight for the comparison. **Segment/report agreement by `DSE`** (fresh → tight agreement; the degradation-with-age curve IS a finding and feeds 8.4). `gp_history` backstop only for the few top-N objects that drift (see Notes).
...
- [ ] **8.5 Tests** — fetcher parsing + comparison logic, against a **saved SOCRATES CSV fixture** (offline/deterministic — mirrors the 6.0 mock-fetch hygiene, no network in CI).
