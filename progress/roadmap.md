...
- [ ] **9.2 Define + build the snapshot** *(the data file — do this first; it's the contract everything downstream reads)* — an offline script: fetch **active payloads from CelesTrak** (~11k) → run the C++ screen (Euclidean + SFS lenses) → write one compact **`snapshot.json`** = orbital **elements** (for in-browser propagation) + conjunction list (TCA, miss, RTN, rel-speed, regime) + freshness metadata (`last_fetched`, epoch ages). **Target ≤ ~5 MB gzipped.** Reuses `tle_fetcher` + `run_screen` unchanged. *(CelesTrak is reachable from a GitHub runner — the TLS block we hit in 8.3 was the local VPN, not a real limit; no Space-Track needed here. The ~117 s / 5.2 GB figure is a dense-Starlink worst case; the real cross-shell ~11k set screens well below it.)*
- [~] **9.3 Point the frontend at the snapshot + scale to ~11k** — read `snapshot.json` instead of the live API; **web-worker `satellite.js` propagation** (throttled cadence + interpolation — full SGP4 on ~11k/frame is too heavy), **Cesium LOD/culling**, **layer toggles** by group/type, "animate only what's needed" (conjunction sats fully tracked, the rest low-cadence), a **"updated X ago" freshness line**, and a **link to the validation report**. ⚠ **Measure browser perf early — if choppy, show fewer sats** (real perf sets the object budget). *(Interactive core already done Jun 22 — click-a-conjunction → fly-to + TCA-5min jump + both trails, RTN/miss/rel-speed/TCA detail panel, clickable list, label de-overlap, LIVE button, `N flagged · M sats` header; see `task_logs/task_9_1_conjunction_ux.md`. Still open: alert-table sort / TCA countdown, severity colors.)*
- [ ] **9.4 Deploy the static site to GitHub Pages** — host the frontend + `snapshot.json` on **GitHub Pages**; **domain-restrict the Cesium Ion token** to the Pages domain; confirm the deployed URL loads with **no call to our old backend**.
- [ ] **9.5 Set up the robot job (GitHub Actions) + snapshot archive** *(the refresh mechanism)* — a scheduled workflow (a few×/day; **public repo** for the free 16 GB / unlimited-minutes runner) that builds the `.so`, runs the 9.2 script, publishes the fresh **`snapshot.json`**, **and appends a compressed `snapshots/<ISO-timestamp>.json.gz`** to an archive (a `data` branch — **never `main`**, to avoid history bloat). This is what keeps the site current *and* shields the upstream sources (no per-visit fetch → no IP-block risk). **Confirm it fires on schedule and updates the live site.** *(Closes `scaling_tracker #2` for prod; local backend keeps manual `POST /api/refresh`. The archive backs the 9.10 evolution view + the Phase-8 "predicted vs. reality" story.)*
- [ ] **9.6 Verify the deployed site — visual + behavior QA** *(NEW)* — on the live URL: satellites render and animate **smoothly**; conjunctions are clickable with correct TCA/miss/RTN detail; the **freshness line + validation-report link** work; layer toggles behave; **no console errors**; it looks right on a normal laptop screen; and — the key check — the browser **network tab shows zero calls to our old backend/CelesTrak on a page visit**. Fix anything off before sharing the link.
- [ ] **9.7 README rewrite** — remove all ML framing; a simple architecture diagram (**robot job → `snapshot.json` → static globe**); screenshots of the live site; the **live URL**; **links to the validation report + `sgp4_uncertainty.md`**; run-locally instructions.
- [ ] **9.8 Demo** — record a short GIF/video walkthrough of the **deployed** site for the portfolio.
- [ ] **9.9 Final cleanup** — dead code removed, docstrings on public APIs, all tests green; **remove the synthetic demo seed** (`ORBITWATCH_DEMO_SEED` + `append_demo_crosser`) now the real active catalog drives the demo — keep `build_synthetic_shell` only if tests still use it.
