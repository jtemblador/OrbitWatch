...
- [ ] **10.1b Spec + build the time filter (the speed half of Stage 1)** — the event-level no-skip contract + conservative bound (plane-distance node windows `|sin u| ≤ D_eff/(r_p·sin I_R)` + reverse-triangle radius-interval gap at both nodes + transit-time interval intersection), margins: mean-vs-osculating (~10 km), relative nodal precession `|Ω̇₁−Ω̇₂|·T` amplified `~1/sin I_R` (secular rates off the satrec's `nodedot`/`argpdot`), `|ω̇|·T`, drift pad. Near-coplanar → keep. **⚠ Validate the TIME windowing no-skip in the prototype FIRST** (10.0 proved the *path* drop safe but only *estimated* the time fraction) — extend `path_gate.py` to emit windows + diff events vs the full scan, THEN build it into `screen_pairs`. Cite Hoots-Crawford-Roehrich 1984 + Alfano smart sieve.
- [ ] **10.3 Validate + re-profile Stage 1** — **byte-identical events** on the dense shell, full Starlink, and full active (the 10.0 prototype is the independent oracle); expected after 1b: medium pair-step work −50–300×, CI point ~2× total. Re-run `profile_screening.py --source active --mode sfs --fused` at 5 k / 10 k vs the 10.0 baselines. *(1a's byte-identical + RSS check already done — see 10.1a.)*
