...
- [ ] **10.3 Validate + re-profile Stage 1** — **byte-identical events** on the dense shell, full Starlink, and full active; expected: medium pair-step work −41× at the CI slice (−130–168× full catalogs), CI point ~1.5–2× total. Re-run `profile_screening.py --source active --mode sfs --fused` at 5 k / 10 k vs the 10.0 baselines. *(1a's byte-identical + RSS check already done — see 10.1a.)*
